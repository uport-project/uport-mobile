...
const findRequest = (state, activityId) => activityId && state.uport
  ? first(
...
export const externalProfile = (state, address) => {
  ...
  if (profileFromVC && profileFromVC.name) {
...
const clientProfileMap = (state) => currentRequest(state) ? externalProfile(state, currentRequest(state).client_id) : null
