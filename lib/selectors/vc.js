...
const claimValue = (key, claims) => {
  ...
  const potentialClaims = filter(claims, claim => claim.payload && claim.payload.claim && claim.payload.claim[key])
...
export const allProfiles = state => {
  const allClaims = getList(state)
  const uniqueSub = _.uniqBy(allClaims, item => item.payload && item.payload.sub)
  const profiles = uniqueSub.map(item => profileForDID(state, item.payload.sub))
  return profiles
}

export const profileForDID = (state, did) => {
  const claims = filter(getList(state), claim => claim.payload && claim.payload.sub && claim.payload.sub === did)
  ...
  if (profileImage) {
    ...
    } else if (typeof profileImage === 'string' || profileImage instanceof String) {
      profile['avatar'] = { uri: profileImage }
    } else {
  ...
  if (bannerImage) {
    if (bannerImage['/']) {
      profile['bannerImage'] = { uri: ipfsUrl + bannerImage['/'] }
    } else {
      profile['bannerImage'] = bannerImage
    }
  }

  const brandColor = claimValue('brandColor', claims)
  if (brandColor) {
    profile['brandColor'] = brandColor
  }
