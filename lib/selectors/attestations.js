...
import { toJs, toClj, hashMap, isEmpty, count, selectKeys, vector, assoc, set, get, getIn, vals, map, reduce, reduceKV, inc, updateIn, sortedMap, first, groupBy, keys, dissoc, filter, reverse, concat, conj } from 'mori'
import { currentIdentity, ownClaims, currentDID, currentDidObject } from './identities'
import { contactList } from './vc'
import _ from 'lodash'

const externalIdentities = state => {
  const external = toJs(get(state.uport, 'external')) || {}
  const vcProfiles = contactList(state)
  _.forEach(vcProfiles, (profile) => {
    const key = profile.did || profile.address
    profile.avatar = profile.profileImage && {uri: profile.profileImage} 
    external[key] = Object.assign(external[key] ? external[key] : {}, profile)
  })
...
export const requestedAttestations = (state, requested) => {
  const requestedSet = set(requested)
  return filter(att => !isEmpty(selectKeys(get(att, 'claim'), requestedSet)), onlyLatestAttestations(state))
}
...
export const attestationLedger = createSelector(
  [currentIdentity],
  (identity, did) => concat(reverse(vals(get(identity, 'attestations'))))
)
