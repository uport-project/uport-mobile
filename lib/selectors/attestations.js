...
import { toJs, merge, toClj, hashMap, hasKey, count, selectKeys, vector, assoc, flatten, get, getIn, vals, map, reduce, reduceKV, inc, updateIn, sortedMap, first, groupBy, keys, dissoc, filter, sortBy, concat, conj } from 'mori'
...
const JWT_MS_CUTOFF = 1451628000000
...
const externalIdentities = createSelector(
  ...
  (ext, vcs) => Object.keys(allProfiles).reduce((all, address) => updateIn(all, [address], p => merge(p, toClj(allProfiles[address]))), ext)
)
