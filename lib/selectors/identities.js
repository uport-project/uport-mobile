...
import { assoc, apply, keys, map, concat, distinct, toClj, first, filter, remove, sort, merge, vals, hashMap, get, getIn, selectKeys, toJs, seq, sortBy, vector } from 'mori'
...
export const allAccounts = createSelector(
  [accountsByAddress],
  (all) => filter(acct => !get(acct, 'disabled') && get(acct, 'address') && get(acct, 'address').slice(0, 4) !== 'temp', vals(all))
)
...
export const migrateableIdentities = createSelector(
  [allIdentities],
  identities => toJs(map(id => selectKeys(id, ['address', 'network', 'signerType']), remove(isDisabled, filter(legacyIdentity, identities))))
)
