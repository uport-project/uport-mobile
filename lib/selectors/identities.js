...
import { assoc, apply, keys, map, reduce, reduceKV, concat, distinct, toClj, first, filter, remove, sort, merge, vals, hashMap, get, getIn, selectKeys, toJs, seq, sortBy, vector } from 'mori'
...
export function normalizeDID (address) {
  if (!address) return
  if (!address.match) return
  if (address.match(/^did:([a-zA-Z0-9]+):([[a-zA-Z0-9.-]+)(\/[^#]*)?(#.*)?$/)) return address
  if (MNID.isMNID(address)) return `did:uport:${address}`
  if (address.match(/^0x[0-9a-fA-F]{40}$/)) return `did:ethr:${address}`
}
...
export const currentDID = createSelector(
  [currentAddress],
  (address) => normalizeDID(address)
)

export const allAccounts = createSelector(
  [accountsByAddress],
  (all) => filter(acct => get(acct, 'address') && get(acct, 'address').slice(0, 4) !== 'temp', vals(all))
)
...
export const rootIdentities = createSelector(
  [allIdentities],
  (identities) => toJs(map(id => normalizeDID(get(id, 'address')), identities))
)
...
export const subAccountsSeq = createSelector(
  ...
  (selected, accounts) => filter(acct => normalizeDID(get(acct, 'parent')) === normalizeDID(selected), accounts)
)
...
export const identityAccounts = createSelector(
  [subAccountsSeq],
  (subidentities) => toJs(map(id => selectKeys(id, ['address', 'deviceAddress', 'network', 'signerType', 'balance']), subidentities))
)

export const myAccounts = createSelector(
  [currentIdentity, mySubAccountsSeq],
  (identity, subaccounts) => toJs(concat(!get(identity, 'address') || get(identity, 'address').match(/^did:/) ? [] : vector(identity), subaccounts))
)
...
export const accountsForNetworkSeq = createSelector(
  ...
  (network, accounts) => filter(acct => decodeAddress(get(acct, 'address')).network === network && !get(acct, 'clientId'), accounts)
)
...
export const accountsForClientIdAndNetworkSeq = createSelector(
  ...
  (network, clientId, accounts) => filter(acct => decodeAddress(get(acct, 'address')).network === network &&
  get(acct, 'clientId') === clientId, accounts)
)
...
export const accountsForClientIdSignerTypeAndNetworkSeq = createSelector(
  ...
  (network, clientId, signerType, accounts) => filter(acct => decodeAddress(get(acct, 'address')).network === network &&
  get(acct, 'clientId') === clientId && get(acct, 'signerType') === signerType, accounts)
)
...
export const validPrimaryAddresses = createSelector(
  [allIdentities],
  (identities) => toJs(map(id => get(id, 'address'), filter(id => get(id, 'network') === defaultNetwork.name && get(id, 'address') !== 'new', identities)))
)
...
export const sharableProfileMapForAddress = createSelector(
  [selectedIdentity],
  (identity) => merge(
    selectKeys(get(identity, 'own'), ['name', 'avatar']),
    selectKeys(identity, ['publicEncKey'])
  )
)
...
export const connections = createSelector(
  ...
  (identities, connected) => {
    return toJs(sort(connectionComparator,
      remove(contractConnection,
        map(address =>
          assoc(
            get(identities, address, hashMap('address', address, '@type', 'Contract')),
            'address', address
          ), connected)
        )
      )
