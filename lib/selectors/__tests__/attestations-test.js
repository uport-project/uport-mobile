...
describe('missingClaims', () => {
  it('handles missing claims specs', () => {
    expect(toJs(missingClaims(withIdentity, [{type: 'cityId', iss: ['did:web:city.claims'], essential: true, reason: 'We need this', url: 'https://city.claims'}, {type: 'name'}]))).toEqual({name: 'Alice', cityId: {type: 'cityId', iss: ['did:web:city.claims'], essential: true, reason: 'We need this', url: 'https://city.claims'}})
    expect(toJs(missingClaims(withIdentity, [{type: 'name', iss: ['did:ethr:0x1bc5cbf71b068642fc5e89c5e3d0bdb3e366bc0a']}]))).toEqual({name: {type: 'name', iss: ['did:ethr:0x1bc5cbf71b068642fc5e89c5e3d0bdb3e366bc0a']}})
    expect(toJs(missingClaims(withIdentity, [{type: 'name', iss: ['did:ethr:0x1bc5cbf71b068642fc5e89c5e3d0bdb3e366bc0b']}, 'nationalId']))).toEqual({name: {type: 'name', iss: ['did:ethr:0x1bc5cbf71b068642fc5e89c5e3d0bdb3e366bc0b']}, nationalId: 'xxx0xxxx0xxx'})
  })
})
