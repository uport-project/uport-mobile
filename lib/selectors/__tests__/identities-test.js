...
import { toJs, toClj, assoc, reduce, vector } from 'mori'
import myInfoReducer from 'uPortMobile/lib/reducers/myInfoReducer';
...
const appSpecificAccountMainNet = {
  network: 'mainnet',
...
const disabledAppSpecificAccountMainNet = {
  network: 'mainnet',
  address: MNID.encode({address: '0xadd7ef5c99be7a454b4a9869683a3a394fde970a', network: '0x1'}),
  clientId,
  disabled: true,
  signerType: 'KeyPair',
  deviceAddress: '0xadd7ef5c99be7a454b4a9869683a3a394fde970a',
  parent: '2osGwhCweF72aBMFFkk59JrFHNdyG9mwrNV',
  controllerAddress: '0x9df0e9759b17f34e9123adbe6d3f25d54b72ad6c',
  createdAt: 1492997058050,
  updatedAt: 1492997058050
...
const appSpecificAccountRopsten = {
  network: 'ropsten',
...
const withAppSpecificIdentities = {
  uport: reduce(uportReducer, null,
    [storeIdentity(simpleIdentity),
      storeSubAccount(appSpecificAccountMainNet),
      storeSubAccount(appSpecificAccountRopsten),
      storeSubAccount(appSpecificAccountKovan),
      storeSubAccount(disabledAppSpecificAccountMainNet)])
}
...
it('allIdentities returns all primary identities', () => {
  ...
  expect(toJs(allIdentities(withMultipleIdentities))).toEqual([simpleIdentity, otherIdentity])
...
it('validPrimaryIdentities returns all primary identities with ethr-dids', () => {
  expect(toJs(validPrimaryIdentities(withIdentity))).toMatchSnapshot()
  expect(toJs(validPrimaryIdentities(withMultipleIdentities))).toMatchSnapshot()
  expect(toJs(validPrimaryIdentities(withSubIdentity))).toMatchSnapshot()
  expect(toJs(validPrimaryIdentities(withMultipleSubIdentities))).toMatchSnapshot()
  expect(toJs(validPrimaryIdentities(withSecondaryOnboarding))).toMatchSnapshot()
})
...
it('subAccounts returns all my sub accounts', () => {
  ...
  expect(subAccounts(withMultipleSubIdentities, '2osGwhCweF72aBMFFkk59JrFHNdyG9mwrNV')).toEqual([subIdentity, subIdentity2])
...
it('myAccounts returns all myself with all my sub identities', () => {
  ...
  expect(myAccounts(withMultipleSubIdentities)).toEqual([simpleIdentity, subIdentity, subIdentity2])
})
...
it('returns app specific identities for given clientId and network', () => {
  ...
  expect(accountForClientIdAndNetwork(withAppSpecificIdentities, '0x1', clientId)).toEqual(appSpecificAccountMainNet)
  expect(accountForClientIdAndNetwork(withAppSpecificIdentities, '0x3', clientId)).toEqual(appSpecificAccountRopsten)
  expect(accountForClientIdAndNetwork(withAppSpecificIdentities, '0x2a', clientId)).toEqual(appSpecificAccountKovan)
...
it('returns app specific identities for given clientId, signerType and network', () => {
  ...
  expect(accountForClientIdSignerTypeAndNetwork(withAppSpecificIdentities, '0x1', clientId, 'KeyPair')).toEqual(appSpecificAccountMainNet)
  expect(accountForClientIdSignerTypeAndNetwork(withAppSpecificIdentities, '0x1', clientId, 'MetaIdentityManager')).toEqual(null)
  expect(accountForClientIdSignerTypeAndNetwork(withAppSpecificIdentities, '0x3', clientId, 'MetaIdentityManager')).toEqual(appSpecificAccountRopsten)
  expect(accountForClientIdSignerTypeAndNetwork(withAppSpecificIdentities, '0x3', clientId, 'KeyPair')).toEqual(null)
  expect(accountForClientIdSignerTypeAndNetwork(withAppSpecificIdentities, '0x2a', clientId, 'MetaIdentityManager')).toEqual(appSpecificAccountKovan)
...
describe('migration', () => {
  describe('ethr-did', () => {
    it('should not be migrateable', () => {
      expect(migrateableIdentities({uport: uportReducer(null, storeIdentity(ethrIdentity))})).toEqual([])
    })
  })

  describe('uport-did', () => {
    it('should be migrateable', () => {
      expect(migrateableIdentities({uport: uportReducer(null, storeIdentity(simpleIdentity))})).toEqual([{address: simpleIdentity.address, signerType: simpleIdentity.signerType, network: simpleIdentity.network}])
    })

    it('it should not include disabled accounts', () => {
      expect(migrateableIdentities({uport: uportReducer(null, storeIdentity({...simpleIdentity, disabled: true}))})).toEqual([])
    })
  })
})

describe('mainnetAccounts()', () => {
  it('should not include mainnet identities', () => {
    expect(toJs(mainnetAccounts({uport: uportReducer(null, storeIdentity(ethrIdentity))}))).toEqual([])
  })
  it('should not include any identities', () => {
    expect(toJs(mainnetAccounts({uport: uportReducer(null, storeIdentity(simpleIdentity))}))).toEqual([])
  })
  it('should only include mainnet sub accounts', () => {
    expect(toJs(mainnetAccounts(withAppSpecificIdentities))).toEqual([appSpecificAccountMainNet])
  })
})

describe('hasMainnetAccounts()', () => {
  it('should not include mainnet identities', () => {
    expect(hasMainnetAccounts({uport: uportReducer(null, storeIdentity(ethrIdentity))})).toBeFalsy()
  })
  it('should not include any identities', () => {
    expect(hasMainnetAccounts({uport: uportReducer(null, storeIdentity(simpleIdentity))})).toBeFalsy()
  })
  it('should only include mainnet sub accounts', () => {
    expect(hasMainnetAccounts(withAppSpecificIdentities)).toBeTruthy()
  })
})
