...
import {
  ...
  importSnapshot,
  resetHub,
} from 'uPortMobile/lib/actions/hubActions'
...
import { migrationTargets } from 'uPortMobile/lib/selectors/migrations'
import { dataBackup } from 'uPortMobile/lib/selectors/settings'

const POLL_FREQUENCY = 60000
const SERVER = 'https://api.uport.me/caleuche'
...
export function * checkForBackup (address) {
  try {
    ...
    if (response.status === 200) {
      const responseJson = yield call(response.json.bind(response))
      return responseJson.data.total > 0
    } else {
      return false
    }
  } catch (error) {
