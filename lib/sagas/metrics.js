...
import { currentNetwork } from 'uPortMobile/lib/selectors/chains'
...
import { ADD_MIGRATION_TARGET, COMPLETED_MIGRATION_STEP, FAILED_MIGRATION_STEP } from 'uPortMobile/lib/constants/MigrationActionTypes'
  ...
  if (segId && !optOut) {
    try {
      ...
      yield call([analytics, analytics.identify], {
        userId: segId,
        traits: { platform: Platform.OS, existingUport: !!address, chain: network },
        context: { ip: "0.0.0.0" }
      })
    } catch (error) {
      console.log(error)
...
function scrubProperties (input) {
  try {
    let properties = Object.keys(input).reduce((obj, key) => {
      ...
      } else if (allowedProperties.includes(key)) {                                                    // C1. Handle anything else that is not an array or object
