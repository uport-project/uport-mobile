...
import {
  startWorking, stopWorking, failProcess, saveMessage
...
import { currentNonce, defaultWeb3, web3ForAddress, web3ForNetwork, networkSettingsForAddress, deviceAddressForAddress, txRelayAddressForAddress } from 'uPortMobile/lib/selectors/chains'
...
export function * fetchFuel ({address}) {
  try {
    ...
    if (isConnected && address) {
      ...
      const fuel = yield call(web3.getBalance, settings.deviceAddress || settings.hexaddress)
...
export function * fetchNonce ({address}) {
  try {
    ...
    if (isConnected && address) {
      const web3 = yield select(web3ForAddress, address)
      const nonce = yield call(web3.getTransactionCount, settings.deviceAddress || settings.hexaddress)
...
export function * resolveNonce ({address}) {
  try {
    ...
    yield put(saveMessage('nonce', address))
