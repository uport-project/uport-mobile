...
jest.mock('react-native', () => {
  ...
  return MockNativeModule(address, privateKey, publicKey)
})

import { deviceAddress, deviceAddresses, keychainSecurityLevel, networkSettingsForAddress } from 'uPortMobile/lib/selectors/chains'
import {
...
describe('createSeed()', () => {
  it('creates and stores a seed', () => {
    ...
    .provide([])
    .call(NativeModules.NativeHDSignerModule.createSeed, DEFAULT_LEVEL)
...
describe('importSeed()', () => {
  ...
  it('imports and stores a seed', () => {
    ...
    .call(NativeModules.NativeHDSignerModule.importSeed, phrase, DEFAULT_LEVEL)
    .put(storeRootAddress(address))
    .returns({
...
describe('createIdentityAddress()', () => {
  ...
  describe('with existing seed', () => {
    ...
    it('extracts address', () => {
      ...
      .put(incIdentityIndex())
      .call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/1'/0'/0'`, 'Create new identity')
      .returns({
        hdindex: 1,
        address,
        publicKey
...
describe('createRecoveryAddress()', () => {
  ...
  it('extracts address with default account path', () => {
    ...
    .provide([
      [select(hdRootAddress), rootAddress],
      [call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/0'/0'/1'`, 'Create new recovery account'), {address: recoveryAddress}]
    ])
    .call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/0'/0'/1'`, 'Create new recovery account')
  ...
  it('extracts address with specific account path', () => {
    ...
    .provide([
      [select(hdRootAddress), rootAddress],
      [call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/2'/3'/1'`, 'Create new recovery account'), {address: recoveryAddress}]
    ])
    .call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/2'/3'/1'`, 'Create new recovery account')
    .returns(recoveryAddress)
    .run()
  })
})
...
describe('createSubAccountAddress()', () => {
  describe('with existing rootAddress', () => {
    ...
    it('extracts address', () => {
      ...
      .call(NativeModules.NativeHDSignerModule.addressForPath, rootAddress, `m/7696500'/0'/2'/0'`, 'Create new account')
      .returns({
        hdindex: 2,
        address,
        publicKey
  ...
  describe('with no existing rootAddress', () => {
    it('creates seed and extracts address', () => {
      ...
      .put(incAccountIndex(0))
      .call(createSeed)
      .call(NativeModules.NativeHDSignerModule.addressForPath, address, `m/7696500'/0'/2'/0'`, 'Create new account')
      .returns({
        hdindex: 2,
        address,
        publicKey
...
describe('createKeyPair()', () => {
  it('creates and stores key', () => {
    ...
    .provide([])
    .call(NativeModules.NativeSignerModule.createKeyPair, DEFAULT_LEVEL)
    .returns({
      address,
      publicKey
    })
    ...
    it('Returns keypair', () => {
      return expectSaga(encryptionKeyPair, { idIndex: 1, actIndex: 2 })
        .provide([
          ...
          [call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/1'/2'/2'`, 'Create new encryption key'), ENC_KEY_PAIR.secret]
        ])
        .call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/1'/2'/2'`, 'Create new encryption key')
        .returns(KP)
        .run()
    })
  })
  ...
  describe('account path values are set', () => {
    it('returns secret key', () => {
      return expectSaga(encryptionSecretKey, { idIndex: 1, actIndex: 2 })
        .provide([
          [select(hdRootAddress), address],
          [call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/1'/2'/3'`, 'Create new encryption key'), ENC_SECRET_KEY]
        ])
        .call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/1'/2'/3'`, 'Create new encryption key')
  ...
  describe('account path values are not yet set', () => {
    it('returns secret key', () => {
      return expectSaga(encryptionSecretKey, { idIndex: undefined, actIndex: undefined })
        .provide([
          [select(hdRootAddress), address],
          [call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/0'/0'/3'`, 'Create new encryption key'), ENC_SECRET_KEY]
        ])
        .call(NativeModules.NativeHDSignerModule.privateKeyForPath, address, `m/7696500'/0'/0'/3'`, 'Create new encryption key')
        .returns(naclutil.decodeBase64(ENC_SECRET_KEY))
        .run()
    })
  })
      ...
      it('returns true', () => {
        return expectSaga(canSignFor, address)
          .provide([
            ...
            [call(NativeModules.NativeSignerModule.allAddresses), [deviceAddress]]
          ])
          .returns(true)
          .run()
      })
    ...
    describe('is not stored in keychain', () => {
      it('returns false', () => {
        return expectSaga(canSignFor, address)
          .provide([
            [select(networkSettingsForAddress, address), {deviceAddress}],
            [call(NativeModules.NativeSignerModule.allAddresses), []]
          ])
          .returns(false)
          .run()
      })
    ...
    describe('HD account', () => {
      describe('is stored in keychain', () => {
        it('returns true', () => {
          return expectSaga(canSignFor, address)
            .provide([
              [select(networkSettingsForAddress, address), {deviceAddress, hdindex: 1}],
              [call(NativeModules.NativeHDSignerModule.listSeedAddresses), [deviceAddress]]
            ])
            .returns(true)
            .run()
        })
      ...
      describe('is not stored in keychain', () => {
        it('returns false', () => {
          return expectSaga(canSignFor, address)
            .provide([
              [select(networkSettingsForAddress, address), {deviceAddress, hdindex: 1}],
              [call(NativeModules.NativeHDSignerModule.listSeedAddresses), []]
            ])
            .returns(false)
            .run()
        })
