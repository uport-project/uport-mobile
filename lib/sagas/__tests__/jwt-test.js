...
describe('signerFor()', () => {
  // describe('pick correct native signing method', () => {
  //   describe('hd enabled account', () => {
  //     it('uses signJwtHD', () => {
  //       sinon.spy(NativeModules.NativeHDSignerModule, 'signJwt')
  //       return expectSaga(createToken, address, {prompt: 'sign attestation'}, {claims: {name: 'Bob'}})
  //         .provide([
  //           [select(hdPathFor, address), `m/7696500'/0'/1'/0'`],
  //           [select(hdRootAddress), device]
  //         ])
  //         // .returns('eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.eyJpc3MiOiIzNHdqc3h3dmR1YW5vN05GQzh1ak5KbkZqYmFjZ1llV0E4bSIsImlhdCI6MTQ4NTMyMTEzMywiY2xhaW1zIjp7Im5hbWUiOiJCb2IifSwiZXhwIjoxNDg1NDA3NTMzfQ.sg1oJ7J_f2pWaX2JwqzA61oWMUK5v0LYVxUp3PvG7Y25CVYWPyQ6UhA7U9d4w3Ny74k7ryMaUz7En5RSL4pyXg')
  //         .run()
  //         .then(result => expect(verifier.verify(result.returnValue)).toBeTruthy())
  //         .then(() => expect(NativeModules.NativeHDSignerModule.signJwt.calledOnce).toBeTruthy())
  //         .then(() => expect(NativeModules.NativeHDSignerModule.signJwt.getCall(0).args[0]).toEqual(device))
  //         .then(() => expect(NativeModules.NativeHDSignerModule.signJwt.getCall(0).args[1]).toEqual(`m/7696500'/0'/1'/0'`))
  //     })
  //   })
  //   describe('legacy (non HD) account', () => {
  //     it('signJwtHD', () => {
  //       sinon.spy(NativeModules.NativeSignerModule, 'signJwt')
  //       return expectSaga(createToken, address, {prompt: 'sign attestation'}, {claims: {name: 'Bob'}})
  //         .provide([
  //           [select(deviceAddressForAddress, address), device],
  //           [select(hdPathFor, address), undefined]
  //         ])
  //         // .returns('eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.eyJpc3MiOiIzNHdqc3h3dmR1YW5vN05GQzh1ak5KbkZqYmFjZ1llV0E4bSIsImlhdCI6MTQ4NTMyMTEzMywiY2xhaW1zIjp7Im5hbWUiOiJCb2IifSwiZXhwIjoxNDg1NDA3NTMzfQ.sg1oJ7J_f2pWaX2JwqzA61oWMUK5v0LYVxUp3PvG7Y25CVYWPyQ6UhA7U9d4w3Ny74k7ryMaUz7En5RSL4pyXg')
  //         .run()
  //         .then(result => expect(verifier.verify(result.returnValue)).toBeTruthy())
  //         .then(() => expect(NativeModules.NativeSignerModule.signJwt.calledOnce).toBeTruthy())
  //         .then(() => expect(NativeModules.NativeSignerModule.signJwt.getCall(0).args[0]).toEqual(device))
  //     })
  //   })
  // })
})
