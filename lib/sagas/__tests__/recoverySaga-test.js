...
describe('START_SEED_RECOVERY', () => {
      ...
      it('is successful', () => {
        return expectSaga(recoverySaga)
          .provide([
            ...
            [call(NativeModules.NativeHDSignerModule.importSeed, seed), {address: root}],
            [call(importSeed, seed), kp],
            [call(lookupAccount, {deviceAddress})]
          ])
          .put(startWorking('recovery'))
          .call(NativeModules.NativeHDSignerModule.importSeed, seed)
  ...
  describe('errors', () => {
    it('invalid seed fails', () => {
      return expectSaga(recoverySaga)
        .provide([
          [call(NativeModules.NativeHDSignerModule.importSeed, seed), undefined]
        ])
        .put(startWorking('recovery'))
        .call(NativeModules.NativeHDSignerModule.importSeed, seed)
