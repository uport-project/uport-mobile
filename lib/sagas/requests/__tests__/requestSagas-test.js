...
import { handleURL, authorize, authorizeAndCallback, messageHandler, findRequestType, generateActivityId, performCallback, postBackToServer, handleClearRequest, handleSelectActivity, returnToApp, selectAccount } from '../index.js'
    ...
    ['me.uport:', 'https://id.uport.me/'].forEach((protocol) => {
      ...
      describe('network request', () => {
        ['net'].forEach((path) => {
          ...
          // console.log(url)
          it(url, () => {
            expect(parseAndFindRequestType(url)).toEqual('net')
          })
        })
      })
...
describe('messageHandler', () => {
  ...
  describe('shareReq', () => {
    it('calls disclosureRequest handler', () => {
      ...
      return expectSaga(messageHandler, {message: jwt})
        .provide([
          [call(verifyToken, jwt), {payload, doc}],
          [call(shareReq.handle, payload, jwt, id), request]
        ])
  ...
  describe('shareResp', () => {
    it('calls connnections handler', () => {
      ...
      return expectSaga(messageHandler, {message: jwt})
        .provide([
          [call(verifyToken, jwt), {payload, doc}],
          [call(contactsReq.handle, payload, jwt, id), request]
        ])
  ...
  describe('ethtx', () => {
    it('calls network settings handler', () => {
      ...
      return expectSaga(messageHandler, {message: jwt})
        .provide([
          [call(verifyToken, jwt), {payload, doc}],
          [call(txReq.handle, payload, jwt, id), request]
        ])
  ...
  describe('chainProv', () => {
    it('calls network settings handler', () => {
      const payload = {type: 'chainProv'}
      const request = {}
      return expectSaga(messageHandler, {message: jwt})
        .provide([
          [call(verifyToken, jwt), {payload, doc}],
          [call(netReq.handle, payload, jwt, id), request]
        ])
        .call(netReq.handle, payload, jwt, id)
        .put(storeActivity({...request, type: 'net', id}))
        .run()
    })
  })
  ...
  describe('attestation', () => {
    it('calls network settings handler', () => {
      ...
      return expectSaga(messageHandler, {message: jwt})
        .provide([
          [call(verifyToken, jwt), {payload, doc}],
          [call(attReq.handle, payload, jwt, id), request]
        ])
  ...
  describe('unified', () => {
    describe('with JWT', () => {
      it('forwards JWT on to messageHandler', () => {
        const jwt = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.eyJpc3MiOiIzNHdqc3h3dmR1YW5vN05GQzh1ak5KbkZqYmFjZ1llV0E4bSIsImlhdCI6MTQ4NTMyMTEzMywiY2xhaW1zIjp7Im5hbWUiOiJCb2IifSwiZXhwIjoxNDg1NDA3NTMzfQ.sg1oJ7J_f2pWaX2JwqzA61oWMUK5v0LYVxUp3PvG7Y25CVYWPyQ6UhA7U9d4w3Ny74k7ryMaUz7En5RSL4pyXg'
        const url = `https://id.uport.me/req/${jwt}?callback_type=callback`
        const payload = {claim: {name: 'bob'}}
        return expectSaga(handleURL, {url})
              .provide([
                [call(messageHandler, {message: jwt, target: undefined, popup: undefined, redirectUrl: undefined, postback: undefined}), 'ok']
              ])
              .put(clearMessage('handleUrl'))
              .call(messageHandler, {message: jwt, target: undefined, popup: undefined, redirectUrl: undefined, postback: undefined})
              .returns('ok')
              .run()
      })
    })
    ...
    describe('transaction request', () => {
      ...
      describe('callback_type', () => {
        it('post becomes postback=true', () => {
          const url = 'me.uport:2oZqgNx3pzzaT5V6xctreExLz21jVYrh9ef?value=123&callback_url=https://demo.uport.me&callback_type=post'
          const parsed = URL(url, true)
          const request = {
            id: '123131',
            postback: true,
        ...
        it('redirect becomes postback=false', () => {
          const url = 'me.uport:2oZqgNx3pzzaT5V6xctreExLz21jVYrh9ef?value=123&callback_url=https://demo.uport.me&callback_type=redirect'
          const parsed = URL(url, true)
          const request = {
            id: '123131',
            postback: false,
    ...
    describe(requestType, () => {
      it('spawns authorizeAndCallback', () => {
        return expectSaga(authorize, {activityId})
          .provide([
            [select(fetchRequest, activityId), request],
    ...
    describe('without callback', () => {
      it('keeps request open', () => {
        ...
        return expectSaga(authorizeAndCallback, request)
          .provide([
            [call(txReq.authorize, request), response]
          ])
  ...
  describe('disclosure', () => {
    describe('without callback', () => {
      it('clears request', () => {
        ...
        return expectSaga(authorizeAndCallback, request)
          .provide([
            [call(shareReq.authorize, request), response]
          ])
...
describe('returnToApp', () => {
  ...
  describe('ios', () => {
    it('returns to https url', () => {
      ...
      return expectSaga(returnToApp, URL('https://mydapp.com/hello?mysite=hello'), {status: 'ok'})
        .provide([
          [call(openExternalUrl, 'https://mydapp.com/hello?mysite=hello#status=ok'), undefined]
        ])
    ...
    it('returns to app url', () => {
      ...
      return expectSaga(returnToApp, URL('app.agreed.stream:hello?mysite=hello'), {status: 'ok'})
        .provide([
          [call(openExternalUrl, 'app.agreed.stream:hello?mysite=hello#status=ok'), undefined]
        ])
  ...
  describe('android', () => {
    it('returns to https url', () => {
      ...
      return expectSaga(returnToApp, URL('https://mydapp.com/hello?mysite=hello'), {status: 'ok'})
        .provide([
          [call(IntentLauncher.startActivity, {action: IntentConstant.ACTION_VIEW, data: 'https://mydapp.com/hello?mysite=hello#status=ok', extra: { 'com.android.browser.application_id': 'com.android.chrome' }}), undefined]
        ])
        .call(IntentLauncher.startActivity, {action: IntentConstant.ACTION_VIEW, data: 'https://mydapp.com/hello?mysite=hello#status=ok', extra: { 'com.android.browser.application_id': 'com.android.chrome' }})
        .run()
    })
  ...
  describe('handling responses', () => {
    ...
    it('does NOT append response to redirect_url if undefined', () => {
      ...
      return expectSaga(returnToApp, URL('app.agreed.stream:hello?mysite=hello'), undefined)
        .provide([
          [call(openExternalUrl, 'app.agreed.stream:hello?mysite=hello'), undefined]
        ])
    ...
    it('does NOT overwrite redirect_url fragments', () => {
      ...
      return expectSaga(returnToApp, URL('app.agreed.stream:hello?mysite=hello#mood=happy'), {status: 'ok'})
        .provide([
          [call(openExternalUrl, 'app.agreed.stream:hello?mysite=hello#mood=happy&status=ok'), undefined]
        ])
...
describe('performCallback', () => {
  ...
  describe('postback set', () => {
    ...
    it('performs postback & redirects if redirect_url set', () => {
      const request = {postback: true, redirect_url: 'app.agreed.stream:hello?mysite=hello', callback_url: 'https://mydapp.com/hello?mysite=hello'}
      return expectSaga(performCallback, request, response)
        .provide([
          [call(postBackToServer, request, response), undefined],
  ...
  describe('postback not set', () => {
    it('defaults to postback & redirect if postback is undefined', () => {
      const request = {redirect_url: 'app.agreed.stream:hello?mysite=hello', callback_url: 'https://mydapp.com/hello?mysite=hello'}
      return expectSaga(performCallback, request, response)
        .provide([
          [call(postBackToServer, request, response), undefined],
          [call(returnToApp, URL(request.redirect_url), undefined), undefined]
  ...
  describe('known postbacks', () => {
    ['https://chasqui.uport.me/api/v1/topic/123123', 'https://chasqui.uport.space/api/v1/topic/123123', 'https://api.uport.me/ethereal/v1/callback/', 'https://api.uport.space/ethereal/v1/callback/'].forEach(callback_url => {
...
describe('handleSelectActivity', () => {
  it('existing request', () => {
    ...
    return expectSaga(handleSelectActivity, {activityId})
      .provide([
        [select(currentRequestId), activityId]
      ])
...
describe('handleClearActivity', () => {
  it('clears correctly', () => {
    return expectSaga(handleClearRequest)
      .put(updateBadgeCount())
      .returns(undefined)
      .call(NavigationActions.dismissLightBox, {})
      .call(NavigationActions.dismissModal, {
...
describe('selectAccount', () => {
  ...
  it('does not change account if it doesn not match the supported network', () => {
    return expectSaga(selectAccount, {activityId, account})
      .provide([
        [select(fetchRequest, activityId), {network: '0x4'}]
      ])
      .not.put(updateActivity(activityId, {account}))
      .run()
  })
