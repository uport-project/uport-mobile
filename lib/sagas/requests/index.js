...
export function * handleURL ({ url, postback, popup, clientId, target, legacyPush }) {
  ...
  if (requestType === 'unified') {
    ...
    if (path && path[1].match(JWT_REGEX)) {
      ...
      if (parsed.query.callback_url) callbackUrl = parsed.query.callback_url
      return yield call(messageHandler, {
        message: path[1], 
        popup, 
        target, 
...
export function * messageHandler ({message, popup, target, redirectUrl, postback, callbackUrl}) {

  // console.tron.log(callbackUrl)
