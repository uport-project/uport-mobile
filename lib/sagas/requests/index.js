...
import ConsensysIcon from 'uPortMobile/lib/components/shared/ConsensysIcon'
  ...
  if (handler && handler.authorize) {
    ...
    if (request.infoType === 'seedRecovery') {
      try {
        ...
        NavigationActions.switchToTab({ tabIndex: 4 })
      } catch (e) {
        console.log(e)
      }
    }
...
export function* authorize({ activityId }) {
  try {
    ...
    // console.log(request)
    yield put(track(`authorize ${request.type}`, { network: request.network, callback_url: request.callback_url, client_id: request.client_id }))
...
export function* handleSelectActivity({ activityId }) {
  ...
  if (!!request.type && (request.type === 'disclosure' || request.type === 'sign')) {
    ...
    NavigationActions.switchToTab({ tabIndex: 0 })
  }
