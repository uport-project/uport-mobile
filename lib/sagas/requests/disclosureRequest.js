...
const ACCOUNT_TYPES = set(['general', 'devicekey', 'segregated', 'keypair', 'none'])
...
export function * handle (payload, jwt) {
  ...
  if (payload.verified) {
    yield put(track('requestVerifiedClaim', {claim: payload.verified.toString()}))
  }
  ...
  if (parsed.query) {
    ...
    if (params.requestToken) {
      ...
      if (payload.type === 'shareReq') {
        ...
        request.requested = toJs(union(set(payload.requested || []), set(payload.verified || [])))
...
export function * findCorrectAccount ({target, client_id, network, actType}) {
  if (actType === 'none') return
  const networkName = networks[network] ? networks[network].name : network
  if (actType !== 'devicekey' && !networks[network].supported) {
    throw new Error(`uPort does not support ${networkName} at the moment`)
  }
  if (actType !== 'devicekey' && actType !== 'keypair' && !networks[network].relayUrl) {
  ...
  if (actType === 'segregated' || actType === 'devicekey') {
    if (client_id) {
  ...
  return {parent: target || current.address}
}
...
export function * disclosureRequest (basicReq, parsed) {
  ...
  try {
    ...
    yield put(track('disclosureRequest', {request}))
    return request
  } catch (error) {
    // console.log(error)
...
function * createNewAccount ({id, actType, target, network, client_id}) {
  ...
  try {
    ...
    if (request.requested) {
      ...
      if (request.verified) {
        ...
        yield put(track('authorizeVerifiedClaim', {claim: request.verified.toString()}))
        if (payload.verified > 0) {
          yield put(track('successDataExchange', {claim: request.verified.toString()}))
        }
      }
      ...
      if (request.pushPermissions) {
        ...
        if (allowed) {
          ...
          const pushToken = yield call(createToken, request.target, pushPayload, {expiresIn: ((2 * WEEK) + DAY), issuer}, `Allow ${counterparty ? counterparty.name : 'service'} to send your push notifications`)
          yield put(updateActivity(request.id, {pushToken}))
          yield put(track('disclosureRequest Allowed Notifications', request))
    ...
    yield put(track('disclosureRequest Authorized', {request: request, authorizedAt}))
...
export function * authorizeAccount (request) {
  ...
  if (request.actType !== 'none') {
    if (request.actType === 'devicekey') {
      payload.dad = yield call(createDeviceKey, {parent: request.target, network: request.network, clientId: request.client_id})
    } else {
