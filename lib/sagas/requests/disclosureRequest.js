...
import { createKeyPairAccount } from '../identitySaga'
...
export function * handle (payload, jwt) {
  ...
  if (payload.claims) {
    request.claims = payload.claims
    if (payload.claims.verifiable) {
      yield put(track('requestVerifiedClaim', {claim: Object.keys(payload.claims.verifiable).join(','), client_id: request.client_id, callback_url: request.callback_url}))
    }
  } else if (payload.verified) {
    request.verified = payload.verified
    yield put(track('requestVerifiedClaim', {claim: payload.verified.join(','), client_id: request.client_id, callback_url: request.callback_url}))
  }

  if (payload.requested) request.requested = payload.requested
  ...
  if (parsed.query) {
    ...
    if (params.requestToken) {
      ...
      if (payload.type === 'shareReq') {
        ...
        if (payload.verified) {
          request.verified = payload.verified
          yield put(track('requestVerifiedClaim', {claim: request.verified.join(','), client_id: request.client_id, callback_url: request.callback_url}))
        }
      ...
      request.claims = {
        user_info: {
          name: null, description: null, location: null
        }
      }
...
export function * authorizeDisclosure (request) {
  try {
    ...
    const counterparty = get(moriProfile, 'name')
    ...
    const own = yield select(requestedClaims, request)
    if (own) payload.own = own
    if (request.req) payload.req = request.req
    const verified = yield select(verifiedClaimsTokens, request)
    if (verified && verified.length > 0) {
      payload.verified = verified
      yield call(authorize, `Do you want to share your information with ${counterparty}?`)
      const claimNames = (request.claims ? Object.keys(request.claims.verifiable || {}) : request.verified).join(',')
      yield put(track('authorizeVerifiedClaim', {claim: claimNames, client_id: request.client_id, callback_url: request.callback_url}))
      yield put(track('successDataExchange', {claim: claimNames, client_id: request.client_id, callback_url: request.callback_url}))
    }
    if (request.pushPermissions) {
      ...
      if (allowed) {
        ...
        const pushToken = yield call(createToken, request.target, pushPayload, {expiresIn: ((52 * WEEK) + DAY), issuer}, `Allow ${counterparty || 'service'} to send your push notifications`)
    ...
    const token = yield call(createToken, request.target, payload, {expiresIn: TOKEN_VALIDITY, issuer}, `Provide requested information to ${counterparty || 'service'}`)
    if (request.client_id) {
