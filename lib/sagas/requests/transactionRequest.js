...
export function * handle (payload, jwt, id) {
  ...
  if (payload.value) request.value = new BN(payload.value.slice(2), 'hex')
  ...
  if (payload.gas) {
    request.gas = new BN(payload.gas.slice(2), 'hex')
  }
  ...
  if (!request.gas) {
    request.gas = (yield call(estimateGas, request)) || new BN('3000000', 10)
  }
  const gasPrice = yield call(fetchGasPrice, {address: request.target})
  if (gasPrice) {
    let txCostProps = {gas: request.gas}
    const gasCostWei = request.gas.mul(gasPrice)
    const gasCost = wei2eth(gasCostWei)
    txCostProps.gasCost = request.gasCost = gasCost
    txCostProps.gasCostWei = request.gasCostWei = gasCostWei
  ...
  request.txCost = request.value ? request.value.add(request.gasCostWei) : request.gasCostWei
...
export function * handleLegacy (request, url) {
  ...
  request.gas = params.gas ? new BN(params.gas.slice(2), '16') : null
  if (params.gasPrice) {
...
function* buildTransaction(request, signerType) {
  ...
  let gas = request.gas || (request.data && request.data !== '0x0' ? GAS_LIMIT : VALUE_GAS_LIMIT)
