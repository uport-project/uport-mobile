...
export function * performGasCalculations (request) {
  const txCostProps = {}
  if (!request.gas) {
    txCostProps.gas = yield call(estimateGas, request) || new BN('3000000', 10)
  }
  const gas = (request.gas ? new BN(request.gas) : txCostProps.gas)
  const gasPrice = (yield call(fetchGasPrice, { address: request.target }))
  txCostProps.gasPrice = gasPrice.toNumber()
  const gasCostWei = gas.mul(gasPrice)
  txCostProps.gasCost = wei2eth(gasCostWei)

  const spotPrice = yield call(fetchEthSpotPrice)
  if (spotPrice) {
    txCostProps.ethSpotPrice = spotPrice
  }
  txCostProps.txCost = (request.value ? new BN(request.value, 10).add(gasCostWei) : gasCostWei).toString(10)
...
export function * handle (payload, jwt, id) {
  ...
  if (payload.value) {
    ...
    request.ethValue = new BN(wei2eth(payload.value), 10).toNumber()
  }
...
export function * handleLegacy (request, url) {
  ...
  if (params.value) {
    ...
    request.ethValue = new BN(wei2eth(params.value), 10).toNumber()
  }
...
export function * buildTransaction (request, signerType) {
  ...
  const gas = `0x${new BN(request.gasPrice || GAS_LIMIT).toString(16)}`
