...
export function * authorize (request) {
  try {
    ...
    if (authorized) {
      yield put(track('txRequest Authorized', {client_id: request.client_id, callback_url: request.callback_url}))
      return yield signTypedData(request)
    } else {
      yield put(track('txRequest Unauthorized', {client_id: request.client_id, callback_url: request.callback_url}))
