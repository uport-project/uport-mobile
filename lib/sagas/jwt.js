...
export function * signerFor (address, prompt) {
  ...
  if (path) {
    ...
    signJwt = partial(NativeModules.NativeHDSignerModule.signJwt, root, path)
  } else {
    const deviceAddress = yield select(deviceAddressForAddress, address)
    signJwt = partial(NativeModules.NativeSignerModule.signJwt, deviceAddress)
  }
  return async (data) => {
    const signature = await signJwt(Buffer.from(data).toString('base64'), prompt || 'sign request')
    return {
      recoveryParam: signature.v - 27,
      r: Buffer.from(signature.r, 'base64').toString('hex'),
      s: Buffer.from(signature.s, 'base64').toString('hex')
    }
  }
}
...
export function * createToken (address, payload, opts = {}) {
  ...
  return yield call([credentials, credentials.signJWT], payload, opts.expiresIn || DAY)
}
