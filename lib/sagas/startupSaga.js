...
import { currentAddress, currentIdentity, ipfsProfile, hasEncryptionKey, otherIdentities } from 'uPortMobile/lib/selectors/identities'
...
export function * primaryAddress () {
  ...
  if (address.match(/did:ethr:0x[0-9a-fA-F]{40}/)) {
    ...
    const primary = others.find(id => id.address.match(/did:ethr:0x[0-9a-fA-F]{40}/))
    if (primary) {
