...
import { savePublicUport, switchIdentity, createEncryptionKey, importSnapshot } from 'uPortMobile/lib/actions/uportActions'
import { checkForNotifications, pollForNotifications, updateBadgeCount, initNotifications, sendLocalNotification } from 'uPortMobile/lib/actions/snsRegistrationActions'
...
import { currentAddress, currentIdentity, ipfsProfile, hasEncryptionKey } from 'uPortMobile/lib/selectors/identities'
import { currentNetwork, fuelToken } from 'uPortMobile/lib/selectors/chains'
...
import { initializeSQLDatabase } from './databaseSaga'
...
import { updateContactList } from 'uPortMobile/lib/actions/vcActions'
...
export function * startApplication () {
  try {
    ...
    yield call(initializeSQLDatabase)
    ...
    if (address) {
      ...
      if (!!hasHDWallet) {
        ...
        if (seedConfirmed !== true && riskNotificationSent !== true) {
          yield put(sendLocalNotification('recoveryNotification', 'me.uport:info?infoType=seedRecovery&screen=backup.seedInstructions', 'week'))
      ...
      //   callback: 'https://testapp.uport.me',
      //   network_id: '0x1', // 0x1 for mainnet, 0x4 for rinkeby, 0x2a for kovan
