...
export function * resetKey (key, hd) {
  if (hd) {
    const phrase = yield call(NativeModules.NativeHDSignerModule.showSeed, key, DEFAULT_LEVEL)
    yield call(NativeModules.NativeHDSignerModule.deleteSeed, phrase)
  } else {
    yield call(NativeModules.NativeSignerModule.deleteKey, key)
  }
}
...
export function * canSignFor (address) {
  ...
  if (typeof settings.hdindex === 'number') {
    ...
    return r.address === settings.deviceAddress
  } else {
    const addresses = yield call(NativeModules.NativeSignerModule.allAddresses)
    return addresses.includes(settings.deviceAddress)
  }
}
