...
function * resetIdentity () {
  ...
  if (authorized) {
    ...
    let seedPhrase = yield select(seed)
    if (!seedPhrase) {
      ...
      if (root) {
        ...
        yield resetKey(seedPhrase, true)
      }
      else if (!root) {
        const deviceAddress = yield select(deviceAddress)
        yield resetKey(deviceAddress, false)
      }
    }
...
export function * createKeyPairAccount ({parent, network, clientId, rpcUrl}) {
  ...
  const address = MNID.encode({address: account.address, network: (networksByName[network] || {}).network_id || network})
  if (clientId) {
    account.clientId = clientId
  }
  if (rpcUrl) {
    account.rpcUrl = rpcUrl
  }
