...
import { NavigationActions } from '../utilities/NavigationActions';
...
export function * performSeedRecovery ({phrase}) {
  try {
    ...
    const kp = yield call(NativeModules.NativeHDSignerModule.importSeed, phrase, level)
    // 
    if (!kp) {
    ...
    yield call(alert, 'Recovery Successful!!!', 'Your Identity was successfully recovered')
