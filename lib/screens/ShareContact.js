...
class ShareContactModal extends React.Component {
  ...
  componentDidMount() {
    const selectedClaims = this.props.userData
      .filter(claim => claim.value)
      .concat(this.props.verifications)
      .map(claim => {
        return {
          ...claim,
          isSelected: true,
  ...
  renderPersonalClaim(claim) {
    const iconName = claim.isSelected
      ...
      <View key={claim.type} style={styles.infoRowContainer}>
        <View style={styles.infoRow}>
          <TouchableOpacity
            ...
            onPress={() => this.toggleClaim(claim.id)}
          >
