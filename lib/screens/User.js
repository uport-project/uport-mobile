...
class User extends React.Component {
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        >
          ...
          >
            ...
            >
              <Text style={{ fontSize: 28, color: colors.grey74 }}>
                {count(this.props.verifications)}
              </Text>
  ...
  renderListItem({ item, index }) {
    const { claimTypeHeader, claimTypeTitle, claimSubject } = parseClaimItem(
      item,
    )
          ...
          <View style={{ flex: 1 }}>
            ...
            {this.state.editMode ? (
              <TextInput
                ...
                value={claim.value}
                onChangeText={value =>
                  this.handleChange({ [claim.type]: value })
                }
              />
            ) : claim.value ? (
              ...
              <Text
                style={[styles.infoContent, { color: '#999' }]}
              >{`None provided`}</Text>
            )}
          </View>
        </View>
  ...
  expirationItem(exp) {
    let expirationDate =
      exp && exp >= 1000000000000
        ? moment.unix(Math.floor(exp / 1000))
        : moment.unix(exp)
  ...
  renderAccountItem({ item }) {
    const accountProfile = item.clientId
      ? this.props.accountProfileLookup(item.clientId)
      : null
    const networkName =
      item.network.charAt(0).toUpperCase() + item.network.slice(1).toLowerCase()
    return (
      <TouchableHighlight
        underlayColor='#EAEAEA'
        onPress={() =>
      ...
      >
        <View style={styles.infoRowContainer}>
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoDecoration}>
              {item.balance && `${wei2eth(item.balance.ethBalance)} ETH`}
            </Text>
            <FeatherIcon
              size={26}
              name='chevron-right'
