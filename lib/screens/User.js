        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={'position'} enabled={true}>
          ...
          >
            ...
            >
              ...
              <Avatar source={this.props.avatar} size={150} />
              <Text style={styles.bannerTitle}>{this.props.name}</Text>
              { ! this.props.address.match(/did:ethr:/) 
              ? <Text style={{color:'red'}}>LEGACY TEST NET</Text>
              : null
              }
              {this.props.others.map(id => 
                <TouchableHighlight onPress={() => this.props.switchIdentity(id.address)} key={id.address}>
                  <Text style={{color: 'white', marginTop: 6, marginBottom: 6}}> + {id.address.match(/did:ethr:/) ? `Main ${id.address.slice(0,20)}` : `Legacy ${id.network} ${id.address.slice(20)}`}</Text>
                </TouchableHighlight>)
              }
            </View>
          ...
          <View style={styles.infoRow}>
            {this.state.editMode && (
              <TouchableOpacity style={{ paddingLeft: 10, paddingRight: 20 }}>
                <FeatherIcon size={20} name='edit' color='red' />
              </TouchableOpacity>
            )}
            <View style={{ flex: 1 }}>
    ...
    return (
      <View key={claim.type} style={styles.infoRowContainer}>
        <View style={styles.infoRow}>
          {this.state.editMode && (
            <TouchableOpacity style={{ paddingLeft: 10, paddingRight: 20 }}>
              <FeatherIcon size={20} name='edit' color='grey' />
            </TouchableOpacity>
          )}
          <View style={{ flex: 1 }}>
