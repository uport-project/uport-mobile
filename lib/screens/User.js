        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={'position'} enabled={true}>
          ...
          >
            ...
            >
              ...
              {!this.props.address.match(/did:ethr:/) ? <Text style={{ color: 'red' }}>LEGACY TEST NET</Text> : null}
              {this.props.others.map(id => (
                <TouchableHighlight onPress={() => this.props.switchIdentity(id.address)} key={id.address}>
                  <Text style={{ color: 'white', marginTop: 6, marginBottom: 6 }}>
                    {' '}
                    +{' '}
                    {id.address.match(/did:ethr:/)
                      ? `Main ${id.address.slice(0, 20)}`
                      : `Legacy ${id.network} ${id.address.slice(20)}`}
                  </Text>
                </TouchableHighlight>
              ))}
            </View>
