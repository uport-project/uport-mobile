...
export class AccountFunding extends React.Component {
    ...
    return (
      <View style={styles.warningMessageContainer}>
        ...
        <Text style={styles.warningMessage}>
          This account is on the test Ethereum network {this.props.network}. Do not send real ETH to this address or you
          will loose it.
        </Text>
  ...
  renderEthAddress() {
    return (
      <View>
        <View>
          <Text style={{ fontSize: metrics.font.size.large, textAlign: 'center', color: colors.grey74 }}>
            Ethereum account
          </Text>
          <Text style={{ fontSize: metrics.font.size.regular, textAlign: 'center', color: colors.grey74 }}>
            {this.props.network ? capitalizeFirstLetter(this.props.network) : null} |{' '}
            {this.props.balance ? this.props.balance : 0} ETH
          </Text>
        </View>
        ...
        <View style={styles.addressTextView}>
          <TouchableOpacity
            style={{ flexDirection: 'column', alignSelf: 'center', alignItems: 'center' }}
            onPress={() => this.copyHandler()}
            style={styles.addressButton}>
            <Text style={styles.address} style={{ color: colors.white }}>
              {this.props.hexAddress}
            </Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity
          ...
          <Text style={{ textAlign: 'center', fontSize: metrics.font.size.medium, color: colors.grey74 }}>
            Share Address
          </Text>
          <Ionicon name={'ios-share'} size={32} color={colors.grey74} />
        </TouchableOpacity>

        {this.props.network !== 'mainnet' ? (
          <View style={{ marginTop: 30 }}>
            <Text style={{ fontSize: metrics.font.size.medium, textAlign: 'center', color: colors.grey74 }}>
              Request test Ether for free
            </Text>
            <View style={{ alignItems: 'center', paddingTop: 10 }}>{this.renderFaucetLink(this.props.network)}</View>
          </View>
        ) : null}
      </View>
    )
    ...
    return (
      <View style={styles.cardContainer}>
        <View style={{ zIndex: 1, elevation: 1, flex: 0, justifyContent: 'flex-end', flexDirection: 'row' }}>
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', flex: 0, padding: 15 }}
            onPress={() => (this.props.close ? this.props.close() : this.close())}>
            <EvilIcon name='x' size={metrics.font.size.large} color={colors.brand} />
          </TouchableOpacity>
        </View>
        <ScrollView
          contentContainerStyle={styles.screenContentContainer}
          refreshControl={<RefreshControl refreshing={!!this.props.working} onRefresh={this.refreshBalanceHandler} />}>
