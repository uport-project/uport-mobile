...
class Contacts extends React.Component {
        ...
        return (
            <TouchableHighlight style={styles.contactItem} underlayColor={colors.grey216} onPress={() => this.props.navigator.push({screen: 'screen.Contact', passProps: {user: item}})}>
                <View style={styles.contactItemContainer}>
                    <View style={{paddingHorizontal: 10}}><Avatar source={item} size={45}/></View>
                    <View style={styles.contactItemContent}>
                        <View style={{flex: 1, paddingVertical: 15}}>
                            <Text style={{fontSize: 18, color: colors.grey74}}>{item.name ? item.name : `${item.address.slice(0, 30)}...`}</Text>
                        </View>
                        <Chevron />
                    </View>
    ...
    renderNoContacts() {
        return (
            <View style={styles.emptyStateContainer}>
                <Text style={{fontSize: 22, fontWeight: 'bold'}}>Text needed</Text>
            </View>
        )
    }
    ...
    render () {
        return count(this.props.contacts)
