        ...
        >
          <View style={styles.content}>
            <Text style={styles.titleText}>
              {this.props.accountProfile
                ? this.props.accountProfile.name
                : 'Ethereum Account'}
            </Text>
...
const mapStateToProps = (state, ownProps) => {
  ...
  const networkName = settings.network
    ? settings.network.charAt(0).toUpperCase() +
      settings.network.slice(1).toLowerCase()
    : null
  const ethBalance =
    settings.balance && settings.balance.ethBalance
      ? wei2eth(settings.balance.ethBalance)
      : 0
  const usdBalance =
    settings.balance && settings.balance.usdBalance
      ? settings.balance.usdBalance
      : 0
