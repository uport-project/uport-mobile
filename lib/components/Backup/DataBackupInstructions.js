...
export class DataBackupInstructions extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.dataBackup !== prevProps.dataBackup) {
      if(this.props.dataBackup && this.props.dataBackup !== 'customUrl') {
        this.pushNextScreen('backup.dataSuccess')
      }
    }
  }
...
/*
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.dataBackup !== nextProps.dataBackup) {
      let nextScreen = 'backup.dataSuccess'

      if(this.props.dataBackup === 'customUrl') {
        console.log("Setting nextscreen state")
        nextScreen = 'backup.dataCustom'
      }

      this.pushNextScreen(nextScreen)
    }
  }*/
  ...
  handleOnContinuePress(eventType='') {
    // console.log("HandlePress: ", eventType)
    authorize('Verify identity to Backup Account')
    .then(authorized => {
      if (authorized) {
        if(eventType === 'customUrl') {
          this.pushNextScreen('backup.dataCustom')
        } else {
  ...
  renderCustomButton() {
    if(!this.props.dataBackup) {
      return (
          <OnboardingButton
          disabled={this.props.working || !this.state.agree}
          onPress={() => this.handleOnContinuePress('customUrl')}
          >
          Custom Backup
          </OnboardingButton>
      )
...
export default connect(mapStateToProps, mapDispatchToProps)(connectTheme(DataBackupInstructions))
