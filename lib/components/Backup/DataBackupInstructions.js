...
export class DataBackupInstructions extends React.Component {
  ...
  pushNextScreen() {
    Navigation.push(this.props.componentId, {
      component: {
        name: SCREENS.BACKUP.DataBackupSuccess,
        options: {
          topBar: {
            visible: false,
          },
  ...
  handleOnContinuePress() {
    authorize('Verify identity to Backup Account').then(authorized => {
      if (authorized) {
        ...
        <KeyboardAwareScrollView overScrollMode='always'>
          <Text p>
            {!this.props.dataBackup
              ? 'Your account is not currently backed up. If you need to recover your account, you will be able to recover your Haven ID and main address, but you will not be able to recover claims, credentials, or app-specific accounts.'
              : "Your account is currently backed up. This means in the event of a recovery you'll be able to recover your Haven ID, application accounts, and associated claims and data. By turning backup off, you will delete the encrypted copy of your data on our backup server and will be able to recover only your Haven ID, but not application accounts, claims or data."}
          </Text>
          <Checkbox
            ...
            labelText={
              !this.props.dataBackup
                ? 'I understand that backing up my account stores an encrypted copy of my claims and data on a central server.'
                : 'I understand that turning backup off means I will not be able to restore app-specific Ethereum accounts or any identity data in the event of recovery.'
            }
          />
        </KeyboardAwareScrollView>
        <Status process='deleteBackup' />
        <Status process='sync' />
        <OnboardingButton disabled={this.props.working || !this.state.agree} onPress={this.handleOnContinuePress}>
