...
const AccountTileSmall = props => {
  return (
    <View elevation={10} style={styles.container}>
      <Avatar
        borderColor={colors.white246}
        source={props.accountProfile || { avatar: require('uPortMobile/assets/images/eth-symbol2.png') }}
      ...
      <View style={styles.balancesView}>
        ...
        <Text style={{ color: colors.grey74 }}> {props.usdBalance ? '$' + props.usdBalance.toFixed(2) : '$0'} </Text>
      </View>
      {props.onRefresh ? (
        <TouchableOpacity style={{ flex: 0, marginLeft: 8, marginLeft: 8 }} onPress={props.onRefresh}>
          <Ionicon name={'ios-refresh'} size={24} color={colors.grey74} />
        </TouchableOpacity>
      ) : null}
    </View>
  )
