...
function actionLink(props) {
  ...
  if (props.destination) return () => Navigation.push(props.componentId, { component: { name: props.destination } })
  if (props.href) return () => Linking.openURL(props.href)
}
const MenuItem = props => {
  const navigate = props.destination
      ...
      <TouchableHighlight onPress={onPress}>
        <View style={[globalStyles.menuItem, props.topBorder && { borderTopWidth: StyleSheet.hairlineWidth }]}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            {navigate ? <ForwardArrow /> : null}
            {props.href ? <ExternalLink /> : null}
          </View>
        </View>
      </TouchableHighlight>
    )
