...
describe('Request Component', () => {
  ...
  it('returns a TransactionCard when the request type is sign', () => {
    const navigator = new global.FakeNavigator()
    const tree = renderer.create(
      <FakeProvider state={emptyState}>
        <ConnectedRequest
    ...
    expect(tree).toMatchSnapshot()
    expect(card).toEqual(<TransactionCard navigator={navigator} />)
  })

  it('returns a DisclosureCard when the request type is disclosure', () => {
    const navigator = new global.FakeNavigator()
    const tree = renderer.create(
      <FakeProvider state={emptyState}>
        <ConnectedRequest
    ...
    const card = this.wrapper.instance().pickCard()
    expect(tree).toMatchSnapshot()
    expect(card).toEqual(<DisclosureCard navigator={navigator} />)
  })
