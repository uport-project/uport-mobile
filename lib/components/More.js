    ...
    return (
      <Menu>
        ...
        <MenuItem title='Dump State' onPress={async () => {
          ...
          // use MainBundlePath for iOS, DocumentDirectorypath for Android
          const rootPath = RNFS.MainBundlePath ? RNFS.MainBundlePath : RNFS.DocumentDirectoryPath
          const dumpFilePath = `${rootPath}/dump.json`
          try {
            console.log('writing to ' + dumpFilePath)
            await RNFS.writeFile(dumpFilePath, JSON.stringify(dumpData), 'utf8')
          } catch(e) {
            console.log('ERROR!', e)
          }

          console.log('look for dump file at ' + dumpFilePath)
          const dumpFileExists = await RNFS.exists(dumpFilePath)
          console.log(`dump file${dumpFileExists ? '' : ' not'} found`)
        }}/>
      </Menu>
    )
  }
