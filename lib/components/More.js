...
import { migrationScreens } from 'uPortMobile/lib/constants/MigrationActionTypes'
    ...
    return (
      <Menu>
        ...
        {this.props.hasHDWallet && <MenuItem
          ...
          navigator={this.props.navigator}
        />}
        {this.props.pendingMigrations.filter(target => migrationScreens[target]).map(target => (
          <MenuItem key={target} title={`Migrate ${target}`} destination={migrationScreens[target]} navigator={this.props.navigator} />
        ))}
