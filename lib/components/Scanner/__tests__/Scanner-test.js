...
jest.mock('react-native-camera', () => {
  ...
  RNC.RNCamera = props => <div {...props}>{props.children}</div>
...
describe('Scanner', () => {
  it('renders as disabled by default', () => {
    const wrapper = global.shallow(<Scanner navigator={new FakeNavigator()} />)
  ...
  it('calls scanURL correctly', () => {
    ...
    const wrapper = global.shallow(<ConnectedScanner navigator={new FakeNavigator()} />, { context: { store: store } })
    wrapper.setState({ scannerEnabled: true, hasCameraPermission: true })
