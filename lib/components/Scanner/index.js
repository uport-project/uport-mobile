    ...
    if (hasCameraPermission === null) {
      ...
      return (
        <SafeAreaView style={styles.container}>
          <RNCamera captureAudio={false} style={styles.camera} onBarCodeRead={this.onBarCodeRead.bind(this)}>
            <View style={styles.header}>
              {this.state.scannerEnabled && (
                <View style={styles.headerMessageContainer}>
                  <ActivityIndicator size='small' color='#00ff00' />
                  <Text style={styles.headerMessageText}>Scanning for QR codes</Text>
                </View>
              )}
              {!this.state.scannerEnabled && (
