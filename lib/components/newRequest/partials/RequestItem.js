...
const RequestItem = (props) => (
  <View style={[styles.row, props.spec ? {backgroundColor: 'rgba(255,240,240,1)'} : {}]}>
    ...
    ? <View
      ...
      { (props.spec.iss || []).filter(iss => iss.url).map(iss => {
        return <View key={iss.url} style={{flex: 0, borderRadius: 4, marginTop: 4, padding: 4, backgroundColor: colors.brand}}><TouchableHighlight onPress={() => Linking.openURL(iss.url)} ><Text style={{textAlign: 'center', color: 'white'}}>Apply at {domain(iss.url)}...</Text></TouchableHighlight></View>
      })
     }
    </View>
    : null) }
