  ...
  renderFunding() {
    if (
      !(
        ...
        (this.props.request.gasCost && (this.props.balance && this.props.balance > this.props.request.gasCost)) ||
        (this.props.balance &&
          this.props.request.value &&
          this.props.balance > formatWeiAsEth(this.props.request.value))
      )
    ) {
      return (
        <View
        ...
        <View>
          <View style={{ backgroundColor: '#F6F5FE', borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
            ...
            {this.props.request.abi ? (
              ...
              >
                <Text style={{ fontSize: metrics.font.size.regular }}>Function</Text>
                <Text style={{ fontSize: metrics.font.size.regular }}>
                  {this.props.request.abi.name.replace(/([A-Z])/g, ' $1').replace(/^./, function(str) {
                    return str.toUpperCase()
                  })}
                </Text>
            ...
            {this.props.signerType === 'KeyPair' && (
              <View>
                ...
                >
                  ...
                  <View>
                    <Text style={{ fontSize: metrics.font.size.regular, textAlign: 'right' }}>
                      {' '}
                      $
                      {this.props.request.ethSpotPrice && this.props.request.gasCost
                        ? (this.props.request.ethSpotPrice * this.props.request.gasCost).toFixed(2)
                        : null}{' '}
                      USD{' '}
                    </Text>
                    <Text style={{ fontSize: metrics.font.size.small, textAlign: 'right' }}>
                      {' '}
            ...
            {this.props.request.canceledAt ? (
              ...
              (this.props.balance &&
                ...
                <View
                  style={{
                    backgroundColor: 'rgba(178, 177, 194, 1)',
                    marginLeft: 8,
                    marginRight: 8,
                    marginTop: 15,
                    marginBottom: 15,
...
const mapStateToProps = state => {
  ...
  if (request) {
    cardTitle =
      !!request && !!request.fnSig && (request.fnSig === setAttributesSig || request.fnSig === setSig)
        ? 'Update Public Haven ID' 
