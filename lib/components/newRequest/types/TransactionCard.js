...
export class TransactionCard extends React.Component {
  ...
  renderFunding () {
    if (!(this.props.signerType !== "KeyPair" || (this.props.request.gasCost && (this.props.balance && this.props.balance > this.props.request.gasCost)) || (this.props.balance && this.props.request.value && this.props.balance > formatWeiAsEth(this.props.request.value)))) {
      return (
        <View style={{borderTopWidth: 1, borderBottomWidth: 1, borderTopColor: '#E0DFE6', borderBottomColor: '#E0DFE6', marginTop: 10, marginBottom: 10, padding: 10, flexDirection: 'row', justifyContent: 'space-between'}}>
          <View>
          ...
          <TouchableOpacity onPress={() => this.copyHandler()}>
...
const mapStateToProps = (state) => {
  ...
  const network = networks[decodedMnid.network].name
  ...
  return {
    ...
    address,
    decodedMnid,
    hexAddress: decodedMnid.address,
    currentIdentity,
