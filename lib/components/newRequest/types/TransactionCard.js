...
import { publicUport, currentAddress } from 'uPortMobile/lib/selectors/identities'
            ...
            <View>
              ...
              {this.props.request.value ? (
                <View
                  ...
                  <Text>Value</Text>
                  <Text>{`${formatWeiAsEth(this.props.request.value.toNumber())} ETH`} </Text>
                </View>
              ) : null}
...
TransactionCard.propTypes = {
  ...
  sufficientFunds: PropTypes.bool
}
...
const mapStateToProps = state => {
  ...
  const balanceWei = networkSettings.balance && networkSettings.balance.ethBalance
  ? new BN(networkSettings.balance.ethBalance, 10) : new BN(0)
  const insufficientFunds = !(request && request.txCost && balanceWei.gt(request.txCost))
