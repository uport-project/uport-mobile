...
export class VerificationCard extends React.Component {
  ...
  checkForMarketingUrl(claim) {
    console.tron.log(claim)
    let marketingUrl = ''

    Object.keys(claim).map(key => {
      if (typeof claim[key] === 'object') {
        Object.keys(claim[key]).map(subbkey => {
          if (subbkey === 'marketingUrl') {
            console.tron.log(subbkey)
            marketingUrl = claim[key][subbkey]
          }
        })
      }
    })

    return marketingUrl.startsWith('https://') ? marketingUrl : null
  }

  render() {
    const showUrl = this.props.issuer && this.props.issuer.url !== undefined
    const marketingUrl = this.checkForMarketingUrl(this.props.verification.claim)

    console.tron.log(marketingUrl)
    ...
    return (
      <View style={styles.cardContainer}>
        ...
        {this.props.showActions && !marketingUrl ? (
          <AcceptCancelGroup
            acceptText={'Accept'}
            cancelText='Delete'
          ...
          <AcceptCancelGroup
            ...
            cancelText='Accept'
            onAccept={() => {
              this.props.authorizeRequest(this.props.request)
