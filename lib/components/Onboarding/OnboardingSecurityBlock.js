...
class OnboardingSecurityBlock extends React.Component {
  ...
  _handleAppStateChange = (nextAppState) => {
    if (this.state.appState.match(/inactive|background/) && nextAppState === 'active') {
      ...
      NativeModules.NativeSignerModule.hasSecureKeyguard()
      .then(hasSecureKeyguard => {
  ...
  render () {
    return (
      <ProcessCard
        ...
        onContinue={() => {NativeModules.NativeSignerModule.launchSecuritySettings()}}
      >
        <Text style={onboardingStyles.title}>Update Security Settings</Text>
        <YieldIcon width={90} height={82} style={styles.icon} />
        <Text style={[textStyles.p, styles.message]}>Uport ID requires that your device is protected by a pin or password.</Text>
        <Text style={[textStyles.p, styles.message]}>Navigate to the security settings for your device and set one to continue.</Text>
      </ProcessCard>
